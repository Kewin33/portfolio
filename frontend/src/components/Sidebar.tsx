...
import { Menu, X, Home, BookOpen, Music, Target, Activity, Settings } from 'lucide-react';
...
export default function Sidebar() {
  ...
  const navItems = [
    ...
    { name: t('timeline'), path: '/timeline', icon: <BookOpen size={24} /> },
    { name: t('chess'), path: '/chess', icon: <Target size={24} /> },
    { name: t('music'), path: '/music', icon: <Music size={24} /> },
    { name: t('studies'), path: '/studies', icon: <BookOpen size={24} /> },
    { name: t('hobbies'), path: '/hobbies', icon: <Settings size={24} /> },
    { name: t('sports'), path: '/sports', icon: <Activity size={24} /> },
  ];
                ...
                {navItems.map((item, i) => (
                  ...
                  >
                    <Link 
                      href={item.path}
                      onClick={() => setIsOpen(false)}
                      className="flex items-center gap-4 p-4 text-lg font-medium text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl transition-all"
                    >
                      {item.icon}
                      {item.name}
                    </Link>
